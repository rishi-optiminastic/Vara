  ...
  return (
    <div className="flex flex-col gap-3 p-3">
      <div className="border-b border-[rgba(10,10,10,0.12)] pb-3 shadow-[0_1px_0_rgba(255,255,255,0.6)]">
        <Link href="/dashboard/ad-groups" className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-[#0A0A0A] transition-colors">
          <ChevronLeftIcon className="size-3" />Ad Groups
        </Link>
        <div className="flex items-start justify-between gap-3 mt-1.5">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-[22px] font-medium tracking-tight text-[#0A0A0A] leading-none truncate">{adGroup.name}</h1>
              <StatusBadge status={adGroup.status} />
            </div>
            <p className="text-[11px] text-muted-foreground mt-1.5">
              Campaign:{" "}
              <Link href={`/dashboard/campaigns/${adGroup.campaign.id}`} className="hover:underline text-[#0A0A0A] font-medium">
...
function Field({ label, value, capitalize }: { label: string; value: string; capitalize?: boolean }): React.JSX.Element {
  return (
    <div>
      ...
      <div className={`text-xs font-medium text-[#0A0A0A] mt-1 truncate ${capitalize ? "capitalize" : ""}`}>{value}</div>
    </div>
  )
}
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="flex flex-wrap gap-1">
          {items.map((it) => (
            <Badge key={it} variant="outline" className={`h-4 px-1.5 text-[9px] bg-white/60 border-[rgba(10,10,10,0.16)] ${capitalize ? "capitalize" : "uppercase"}`}>{it}</Badge>
          ))}
        </div>
      )}
    </div>
