...
import { AdForm } from "@/components/ads/components/AdForm"

interface Props {
  searchParams: Promise<{ campaign?: string }>
}

export default async function NewAdPage({ searchParams }: Props): Promise<React.JSX.Element> {
  ...
  return (
    <div className="flex flex-col gap-3 p-3">
      <div className="flex items-baseline justify-between border-b border-[rgba(55,50,47,0.12)] pb-2 shadow-[0_1px_0_rgba(255,255,255,0.6)]">
        ...
        <p className="text-[10px] text-muted-foreground">Creative · Preview</p>
      </div>
      <AdForm campaigns={campaigns} defaultCampaignId={defaultCampaignId} />
    </div>
  )
}
