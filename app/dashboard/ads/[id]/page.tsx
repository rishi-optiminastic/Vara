...
const FORMAT_STYLE: Record<string, string> = {
  BANNER: "bg-[#ECEAE2] text-[#1F40CD]",
  HTML5: "bg-[#ECEAE2] text-[#1F40CD]",
  VIDEO: "bg-[#ECEAE2] text-[#1F40CD]",
  NATIVE: "bg-[#ECEAE2] text-[#1F40CD]",
}
  ...
  return (
    <div className="flex flex-col gap-3 p-3">
      <div className="border-b border-[rgba(10,10,10,0.12)] pb-3 shadow-[0_1px_0_rgba(255,255,255,0.6)]">
        <Link href="/dashboard/ads" className="inline-flex items-center gap-1 text-[11px] text-muted-foreground hover:text-[#0A0A0A] transition-colors">
          <ChevronLeftIcon className="size-3" />Ads
        </Link>
        <div className="flex items-start justify-between gap-3 mt-1.5">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h1 className="text-[22px] font-medium tracking-tight text-[#0A0A0A] leading-none truncate">{creative.name}</h1>
            ...
            <p className="text-[11px] text-muted-foreground mt-1.5">
              Campaign:{" "}
              <Link href={`/dashboard/campaigns/${creative.campaign.id}`} className="hover:underline text-[#0A0A0A] font-medium">
        ...
        <Card className="py-0 gap-0 border-[rgba(10,10,10,0.12)] shadow-[0_1px_0_rgba(255,255,255,0.6),0_4px_12px_-8px_rgba(10,10,10,0.08)]">
          ...
          <CardContent className="p-3.5 grid grid-cols-2 gap-x-6 gap-y-3">
            ...
            <div className="col-span-2">
              ...
              <a href={creative.assetUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-[#0A0A0A] mt-1 truncate block hover:underline font-mono" title={creative.assetUrl}>
            ...
            <div className="col-span-2">
              ...
              <a href={creative.clickUrl} target="_blank" rel="noopener noreferrer" className="text-xs font-medium text-[#0A0A0A] mt-1 truncate block hover:underline font-mono" title={creative.clickUrl}>
                {creative.clickUrl.length > 60 ? `${creative.clickUrl.slice(0, 58)}…` : creative.clickUrl}
              </a>
            </div>
            {creative.walletConnectCta && (
              <div className="col-span-2">
                <Badge variant="outline" className="h-4 px-1.5 text-[9px] bg-[#ECEAE2] text-[#1F40CD] border-[rgba(10,10,10,0.12)]/20">
...
function Field({ label, value }: { label: string; value: string }): React.JSX.Element {
  return (
    <div>
      ...
      <div className="text-xs font-medium text-[#0A0A0A] mt-1 truncate">{value}</div>
    </div>
  )
}
