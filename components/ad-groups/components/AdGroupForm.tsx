  ...
  return (
    <button
      ...
      className={`group flex flex-1 items-center gap-1.5 rounded-md px-2 py-1.5 text-left transition-colors ${
        ...
        : done ? "text-[#0A0A0A] hover:bg-[#0A0A0A]/[0.06]"
        : reachable ? "text-muted-foreground hover:bg-[#0A0A0A]/[0.04]"
        : "text-muted-foreground/50 cursor-not-allowed"
      }`}
    >
      <div className={`size-5 shrink-0 rounded-full flex items-center justify-center ${
        active ? "bg-[#ECEAE2]/15 text-[#FFFFFF]"
        : done ? "bg-[#0A0A0A] text-[#FFFFFF]"
  ...
  return (
    <div className="flex flex-col gap-3 max-w-2xl">
      ...
      <div className="flex items-center justify-between border-t border-[rgba(10,10,10,0.08)] pt-2.5">
        ...
        {step < 2 ? (
          <Button size="sm" className="h-8 gap-1.5 text-xs rounded-full px-5 bg-[#0A0A0A] text-[#FFFFFF] hover:bg-[#0A0A0A] shadow-[inset_0_1px_0_rgba(255,255,255,0.18),0_1px_2px_rgba(10,10,10,0.18)]" onClick={next}>
