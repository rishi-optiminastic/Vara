  ...
  return (
    <Card className="py-0 gap-0 border-[rgba(10,10,10,0.12)] shadow-[0_1px_0_rgba(255,255,255,0.6),0_4px_12px_-8px_rgba(10,10,10,0.08)]">
      ...
      <CardContent className="p-3.5">
        {contracts.length === 0 ? (
          <div className="rounded-xl border border-dashed border-[rgba(10,10,10,0.18)] bg-white/50 px-4 py-8 text-center">
            <p className="text-xs font-medium text-[#0A0A0A]">No contracts tracked</p>
          ...
          <ul className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
            {contracts.map((addr) => (
              <li
                key={addr}
                className="flex items-center justify-between gap-2 rounded-full border border-[rgba(10,10,10,0.12)] bg-white/60 px-3 py-1.5 hover:bg-white transition-colors"
              >
                <span className="font-mono text-[11px] text-[#0A0A0A] tabular-nums truncate" title={addr}>
                  {shorten(addr)}
                </span>
                <Badge
                  variant="outline"
                  className="h-3.5 px-1 text-[8px] uppercase tracking-widest bg-[#ECEAE2] border-transparent text-[#0A0A0A] shrink-0"
                >
                  {primaryChain}
                </Badge>
              </li>
